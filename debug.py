...
def serve(host = '0.0.0.0', port = 50001, on_top=True, center = False, no_detach = False):
    if on_top:
...
def usage():
    ...
    parser.add_argument('-nt', '--no-top', action = 'store_false', help = 'dont always show if receive data')
    if len(sys.argv) == 1:
        print("\n")
        parser.print_help()
        try:
        ...
        if args.cleanup:
            ...
            try:
                serve(args.host, args.port, args.on_top or args.no_top, args.center, args.no_detach)
            except KeyboardInterrupt:
                sys.exit()
